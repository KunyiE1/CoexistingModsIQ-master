...
ProteoAnno::ProteoAnno(const ModPtrVec &fix_mod_ptr_vec,
  ...
  for (size_t i = 0; i < var_mod_ptr_vec_.size(); i++) {
    ...
    ProteoAnno::ProteoAnno(const ModPtrVec &fix_mod_ptr_vec,
                           const ProtModPtrVec &prot_mod_ptr_vec,
                           const ModPtrVec &var_mod_ptr_vec,
                           const ModPtrVec &N_mod_ptr_vec) {
        fix_mod_ptr_vec_ = fix_mod_ptr_vec;
        prot_mod_ptr_vec_ = prot_mod_ptr_vec;
        var_mod_ptr_vec_ = var_mod_ptr_vec;
        N_mod_ptr_vec_ = N_mod_ptr_vec;
        is_nme_ = false;
        for (size_t i = 0; i < var_mod_ptr_vec_.size(); i++) {
            ResiduePtr res_ptr = var_mod_ptr_vec[i]->getModResiduePtr();
            AminoAcidPtr acid_ptr = res_ptr->getAminoAcidPtr();
        ...
        for (size_t i = 0; i < N_mod_ptr_vec_.size(); i++) {
            ResiduePtr res_ptr = N_mod_ptr_vec[i]->getModResiduePtr();
            AminoAcidPtr acid_ptr = res_ptr->getAminoAcidPtr();
            if (N_ptm_map_.find(acid_ptr) == N_ptm_map_.end()) {
                ResiduePtrVec cur_vec;
                cur_vec.push_back(res_ptr);
                N_ptm_map_[acid_ptr] = cur_vec;
            } else {
                N_ptm_map_[acid_ptr].push_back(res_ptr);
            }
        }
    }
    ...
    ProteoAnno::ProteoAnno(const ModPtrVec &fix_mod_ptr_vec,
                           const ModPtrVec &Nterm_fix_mod_ptr_vec,
                           const ProtModPtrVec &prot_mod_ptr_vec,
                           const ModPtrVec &var_mod_ptr_vec,
                           const ModPtrVec &N_mod_ptr_vec) {
        fix_mod_ptr_vec_ = fix_mod_ptr_vec;
        N_fix_mod_ptr_vec_ = Nterm_fix_mod_ptr_vec;
        prot_mod_ptr_vec_ = prot_mod_ptr_vec;
        var_mod_ptr_vec_ = var_mod_ptr_vec;
        N_mod_ptr_vec_ = N_mod_ptr_vec;
        is_nme_ = false;
  ...
  if(N_fix_mod_ptr_vec_.empty()){
      residue_ptr_vec = residue_util::convertStrToResiduePtrVec(seq, fix_mod_ptr_vec_);
  }else{
      residue_ptr_vec = residue_util::convertStrToResiduePtrVec(seq, fix_mod_ptr_vec_, N_fix_mod_ptr_vec_);
  }
  res_vec_2d_.clear();
  shift_vec_2d_.clear();
  // input and fixed mod
  ...
  //N-term ptms
//    for (size_t i = 0; i < residue_ptr_vec.size(); i++) {
        AminoAcidPtr acid_ptr = residue_ptr_vec[0]->getAminoAcidPtr();
        // if exist modified residues
        if (N_ptm_map_.count(acid_ptr)) {
            ResiduePtrVec mod_res_vec = N_ptm_map_[acid_ptr];
            // remove duplications
            ResiduePtrVec exist_res_vec = res_vec_2d_[0];
            for (size_t j = 0; j < mod_res_vec.size(); j++) {
                bool found = false;
                for (size_t k = 0; k < exist_res_vec.size(); k++) {
                    if (mod_res_vec[j] == exist_res_vec[k]) {
                ...
                if (!found) {
                    res_vec_2d_[0].push_back(mod_res_vec[j]);
                    shift_vec_2d_[0].push_back(AlterType::VARIABLE->getId());
                }
            }
        }
//    }
