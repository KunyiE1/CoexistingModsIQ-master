...
class ProteoGraph {
 public:
  ...
  const DistVec2D& getDistVec2D() {return dist_vec_;}
    ...
    std::set<int>getPrecMassSet(){return prec_mass_set_; }



 private:
    std::set<int> prec_mass_set_;

  ProteoformPtr db_proteo_ptr_;

  double nterm_fix_shift_;
  ...
  void compAddedPrecs(int max_mod_num, int max_ptm_sum_mass);
};
