...
PrmPeakPtrVec getPrmPeakPtrs(const PrmMsPtrVec &prm_ms_ptr_vec,
  ...
  return peak_list;
    ...
    PrmPeakPtrVec getPrmPeakPtrs_addPrecs(const PrmMsPtrVec &prm_ms_ptr_vec,
                                 PeakTolerancePtr tole_ptr, std::set<int> & prec_mass_set, double convert_ratio) {
        PrmPeakPtrVec peak_list;
        ...
        // add zero
        double prec_mass = prm_ms_ptr_vec[0]->getMsHeaderPtr()->getPrecMonoMass();
        std::cout<<prec_mass<<std::endl;
        // use spec_id = 0 and score = group_spec_num (size of prm_ms_ptr_vec)
        PrmPeakPtr zero_prm_ptr = prm_peak_factory::getZeroPeakPtr(0, prec_mass, tole_ptr, prm_ms_ptr_vec.size());
        peak_list.push_back(zero_prm_ptr);

        //add new prec mass
        for (auto it=prec_mass_set.begin(); it != prec_mass_set.end(); it++){
            double added_precmass = (*it) / convert_ratio;
            if(added_precmass < (prec_mass - 18.0105)) {
                std::cout<<added_precmass<<std::endl;
                PrmPeakPtr added_prec_prm_ptr = prm_peak_factory::getAddedPrecPeakPtr(0, added_precmass, tole_ptr,
                                                                                      prm_ms_ptr_vec.size());
                peak_list.push_back(added_prec_prm_ptr);
            }
        }
