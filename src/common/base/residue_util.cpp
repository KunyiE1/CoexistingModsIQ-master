...
void applyFixedMod(ResiduePtrVec &residue_ptrs, const ModPtrVec &fix_mod_ptr_vec, const ModPtrVec &N_fix_mod_ptr_vec) {
  for (size_t i = 0; i < residue_ptrs.size(); i++) {
    if(i == 0){
      for (size_t j = 0; j < N_fix_mod_ptr_vec.size(); j++) {
        if (residue_ptrs[i] == N_fix_mod_ptr_vec[j]->getOriResiduePtr()) {
          residue_ptrs[i] = N_fix_mod_ptr_vec[j]->getModResiduePtr();
          break;
        }
      }
    }else {
...
ResiduePtrVec convertStrToResiduePtrVec(const std::string & seq,
                                        const ModPtrVec &fix_mod_ptr_vec, const ModPtrVec &N_fix_mod_ptr_vec) {
  ResiduePtrVec residue_ptrs = convertStrToResiduePtrVec(seq);
  applyFixedMod(residue_ptrs, fix_mod_ptr_vec, N_fix_mod_ptr_vec);
  return residue_ptrs;
}
