...
std::vector<ModPtrVec> readModTxt(const std::string &file_name) {
  ...
  std::vector<ModPtrVec> mod_ptr_vec2d(4);
...
ModPtrVec geneNtermFixedModList(const std::string &str) {
    try {
        if (str == "" || str == "C57" || str == "C58") {
            ModPtrVec mod_ptr_vec;
            return mod_ptr_vec;
        } else {
            return readModTxt(str)[3];
        }
    } catch (const char *e) {
        std::cout << "[Exception]" << std::endl;
        std::cout << e << std::endl;
...
std::vector<alignCasePtr> readDiagonalFile(const std::string &file_name){
  std::ifstream infile(file_name.c_str());
  if (!infile.is_open()) {
    LOG_ERROR("Variable PTM file " << file_name <<  "can not be opened!");
    exit(EXIT_FAILURE);
  }
  std::vector<alignCasePtr> diagInfoVec;
  std::string line;
  while (std::getline(infile, line)){
    if (line.size() == 0) continue;
    if (line[0] == '#') continue;
    ...
    try{
      std::vector<std::string> l = str_util::split(line, ";");
      if (l.size() < 5) throw "The number of information is less than 5.";
      int spec_id;
      std::string prot_name;
      std::string prot_desc;
      short prot_start;
      short spec_start;

      try {
        spec_id = std::stod(l[0]);
      }
      catch(std::invalid_argument& e){
        throw "Error in spec_id.";
      }
      try {
        prot_start = std::stod(l[3]);
      }
      catch(std::invalid_argument& e){
        throw "Error in protein starting position.";
      }
      try {
        spec_start = std::stod(l[4]);
      }
      catch(std::invalid_argument& e){
        throw "Error in spectrum starting position.";
      }
      prot_name = l[1];
      prot_desc = l[2];
      
      alignCasePtr caseInfo = std::make_shared<alignCase>(std::make_tuple(spec_id, prot_name, prot_desc, prot_start, spec_start));
      diagInfoVec.push_back(caseInfo);

    } catch (char const* e) {
      std::cout << "Errors in the Diagonal Information file: "
          << file_name << std::endl
          << "Please check the line" << std::endl
          << "\t" << line << std::endl
          << "Error message: " << e << std::endl;
